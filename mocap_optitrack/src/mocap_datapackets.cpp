...
const geometry_msgs::PolygonStamped RigidBody::get_ros_markerSet(int numberOfMarkers)
{
    geometry_msgs::PolygonStamped ros_markerSet;
    ros_markerSet.header.stamp = ros::Time::now();
    ros_markerSet.polygon.points.reserve(numberOfMarkers);
    for (int j=0 ; j<numberOfMarkers ; j++)
    {
        geometry_msgs::Point32 ros_marker;
        ros_marker.x = marker[j].positionX;
        ros_marker.y = - marker[j].positionZ;
        ros_marker.z =  marker[j].positionY;
        ros_markerSet.polygon.points.push_back(ros_marker);
    }
  return ros_markerSet;
}
